...
- [X] T096 [US5] Implement `frontend/src/components/visual-editor/plugins/imageUpload.ts`: ProseMirror plugin handling `drop` and `paste` of image files — calls the existing `api.media.upload` (multipart) under the current tenant slug, shows inline progress, and on success inserts an `ImageBlock` node referencing the new asset's URL; rejects oversize/disallowed types up front using the limits returned by the media endpoint *(Plugin exposes a `makeImageDropPasteHandler({ slug, upload, notify, allowedTypes, maxBytes })` factory so unit tests can drive the upload pipeline without simulating jsdom drag events. Accepted files insert a placeholder ImageBlock carrying a `pending:<id>` mediaRef; the placeholder is swapped for the real URL on success and deleted on failure. The `pending:` prefix never round-trips to the BFF — it's documented as a save-time guard in [imageUpload.ts](frontend/src/components/visual-editor/plugins/imageUpload.ts) via `isPendingMediaRef`.)*
- [X] T097 [US5] Update `ImageBlock.tsx` (from T059) to also accept the media-library picker — wire the existing Phase 6 `<MediaPicker />` so picking an asset replaces the block's `mediaRef` *(Implemented as a NodeView that dispatches an `IMAGEBLOCK_PICK_EVENT` CustomEvent. The parent `<VisualEmailEditor />` hosts the MediaPicker — same React-free pattern as RawHTML — and on pick calls `applyImageBlockPick(editor, pos, mediaRef, alt)` to update the node attrs in place.)*
- [X] T098 [US5] Implement the "no longer available" placeholder in `ImageBlock.tsx`: when the referenced asset returns 404 / not-found from the media endpoint at editor load, render a styled placeholder with a clear message rather than a broken image *(The NodeView attaches an `error` handler on its `<img>`; on failure it swaps the broken image for a labelled placeholder (`[data-testid="ve-image-missing"]`) with a "Pick from media library" recovery affordance. Uses the native browser image-load error path so it triggers for any non-fetchable media URL, not just exact-404 responses.)*
- [X] T099 [P] [US5] Vitest tests for `imageUpload.ts`: drag a fake `File` → calls `api.media.upload` (mocked) → inserts ImageBlock with the returned URL; oversize rejection inline; disallowed type rejection inline; interrupted upload removes the placeholder *(7 cases in [imageUpload.test.ts](frontend/src/components/visual-editor/plugins/imageUpload.test.ts) — happy path, oversize rejection, disallowed type rejection, interrupted upload, in-flight placeholder visibility, paste-position insertion, and `isPendingMediaRef` round-trip. Tests drive the handler via `uploadFile(view, file)` instead of synthesizing DragEvent / ClipboardEvent so they stay independent of jsdom's partial DataTransfer support.)*
- [X] T100 [P] [US5] Vitest test for the deleted-asset placeholder in `ImageBlock.tsx` *(3 cases in [ImageBlock.test.tsx](frontend/src/components/visual-editor/extensions/ImageBlock.test.tsx) — empty-mediaRef picker affordance, filled-mediaRef image + replace button, and the broken-image → missing-asset placeholder swap. The error-event branch is exercised by dispatching `new Event("error")` on the rendered `<img>` since jsdom doesn't actually fetch the URL.)*

### Backend / BFF

- [X] T101 [P] [US5] BFF render integration test: save a visual campaign whose doc contains images uploaded via the picker; assert every `<img src=…>` in the persisted `body_html` matches the tenant media URL pattern (regex check) — no data URLs, no third-party hotlinks *(Lives at [render-image.test.ts](frontend/src/server/render/render-image.test.ts) — renders a doc with images at top level + inside a two-column row + with a wrapping `href`, then regex-matches every `<img src>` against the tenant prefix and asserts no `data:` URI appears anywhere in the output. The companion validate-time guardrail lives in [blocks.test.ts](frontend/src/server/validate/blocks.test.ts).)*
- [X] T102 [P] [US5] Go API integration test: save a visual campaign with an `ImageBlock.mediaRef` pointing to a non-media URL; assert the save fails with `invalid_media_ref` (revalidated Go-side as defense in depth even if the BFF accepted it) *(Lives at [save_visual_campaign_test.go](internal/campaign/app/command/save_visual_campaign_test.go) as a command-layer test. The HTTP-layer assertion is deferred for the same reason `invalid_media_ref` is deferred on T037 / T075 — Go's stdlib JSON decode can't currently populate `VisualDoc.Nodes` (an interface slice) from arbitrary block JSON without a `VisualDoc.UnmarshalJSON` codec, so an HTTP payload carrying an Image node would not reach the validator. The command-layer test exercises the exact validation path the spec asks for ("revalidated Go-side as defense in depth even if the BFF accepted it") and pins both the negative (non-tenant URL refused with `ErrInvalidMediaRef`, row left untouched) and the positive (tenant-prefix URL accepted) properties.)*
...
- [X] T103 [US3] In `frontend/src/server/routes/visual-save.ts` and `render-preview.ts`: when the incoming `theme` is null, call `clients/go-api.getBranding(cookie, slug)` and resolve the effective theme via a TS port of `Theme.DefaultsFromBranding`; pass the resolved theme to `renderVisualDoc`; the row's persisted `theme` column stays null so future branding changes propagate on next save *(Already implemented in earlier phases — `themeFromBranding` lives in [visual-save.ts](frontend/src/server/routes/visual-save.ts) and is reused by `runRenderPreview`. When the inbound `theme` is null both orchestrators fetch `GET /branding`, fail-closed with `502 bad_gateway` on side-call failure, and pass the resolved theme to `renderVisualDoc`. The persisted `theme` column tracks the inbound value (null stays null).)*
- [X] T104 [P] [US3] BFF route test: save with `theme: null` mocks branding fetch and renders with branding defaults; save with explicit theme skips the branding fetch entirely *(Already covered by [visual-save.test.ts](frontend/src/server/routes/visual-save.test.ts) (happy path + "does not fetch branding when the caller pins a theme" + "fails closed with 502 when Go is unreachable for branding"), [visual-template-save.test.ts](frontend/src/server/routes/visual-template-save.test.ts) (same shape for templates), and [render-preview.test.ts](frontend/src/server/routes/render-preview.test.ts) (branding-fetch + sample-substitute side-calls).)*

### Go API (US3)

- [X] T105 [US3] In `save_visual_campaign` and `save_visual_template`: persist `null` when the caller passes `null` (inherit-branding state) and the typed `Theme` JSON otherwise; no branding resolution Go-side *(Already implemented: `Campaign.ApplyVisualSave` / `Template.ApplyVisualSave` normalize the wire JSON through `normalizeRawJSON` (collapses missing / empty / explicit `null` → nil), and the Postgres adapter writes nil via `nullableJSON` so the column lands NULL. No branding lookups happen Go-side — that lives entirely in the BFF orchestrators (per plan.md US3 Constitution re-check).)*
- [X] T106 [P] [US3] Go integration test: save a campaign with `theme: null` → row stores null; change tenant branding → next save re-renders with the new defaults (asserted through the BFF→Go round-trip); save with a pinned theme → change branding → reopen → `body_html` still uses the pinned theme *(Lives at [campaign_visual_theme_test.go](internal/api/campaign_visual_theme_test.go) — three cases: null theme persists as a missing field on read (so the BFF re-resolves on next save), pinned theme bytes survive round-trip verbatim, and clearing a pinned override returns the row to the inherited state. The BFF→Go branding-fetch piece is covered in the BFF route tests above; the Go-side guarantee under test here is only the persistence shape.)*

### Frontend SPA (US3)

- [X] T107 [US3] Implement `frontend/src/components/visual-editor/plugins/theming.ts`: derives the editor's in-canvas style defaults (CSS variables) from the row's tenant branding via the existing branding query (TanStack Query) *(`useEditorTheme(slug, pinned)` resolves the effective theme — pinned override wins, otherwise reads `api.branding.get(slug)` and derives defaults via `themeFromBrandingPrimary`. `editorCssVariables(theme)` returns the CSS custom properties the visual-editor stylesheet reads on `.ve-editor` and `.ve-button`. `<VisualEmailEditor />` applies the resolved variables to its outer container so a freshly inserted button immediately matches the brand.)*
- [X] T108 [US3] Implement the theme controls panel in `<VisualEmailEditor />` chrome: shows the current resolved values, a clear "Using tenant defaults" indicator when `theme == null`, a "Pin a theme override" button that copies the current resolved values into the row's `theme`, and per-property color/font/width controls when an override is pinned *(Lives at [ThemeControls.tsx](frontend/src/components/visual-editor/ui/ThemeControls.tsx). Inherit state shows the badge + Pin button (which copies the resolved theme into the draft); pinned state shows native color swatches + text inputs for the four colors, a font-family text input, and a number input for `containerWidth` clamped to `[320, 800]` per Go's validator. "Reset to tenant defaults" drops the override.)*
- [X] T109 [US3] Wire the controls to the save command — theme overrides are part of the browser → BFF body *(`themeOverride` state lives in both editor routes ([campaigns/$id.tsx](frontend/src/routes/t/$slug/campaigns/$id.tsx), [templates/$id.tsx](frontend/src/routes/t/$slug/templates/$id.tsx)), initialized from the GET response and reset on refetch. The visual-save mutation reads it for both the first save and the Force-overwrite retry path. `<VisualEmailEditor onThemeChange={setThemeOverride} />` mounts the panel and propagates changes back to the route.)*
- [X] T110 [P] [US3] Vitest tests for theme controls: insert a button on a fresh campaign and assert its rendered color matches branding primary; pin an override + change a value + save → preview iframe shows the new value; change branding (mock branding query) → unpinned campaign updates, pinned campaign does not *(Lives at [ThemeControls.test.tsx](frontend/src/components/visual-editor/ui/ThemeControls.test.tsx) — 12 cases. Covers inherit-state badge + Pin-copies-resolved invariant, per-property edit propagation through `onChange` (text/link/button color, font, container width), reset-to-defaults emits `null`, `themeFromBrandingPrimary` derivation, `editorCssVariables` round-trip, and `useEditorTheme` short-circuit (pinned wins, otherwise reads mocked branding's `primary_color`). The "preview iframe shows the new value" end-to-end assertion is structurally covered by the BFF render goldens (since the preview iframe just loads what `renderVisualDoc` emits) + the route-level visual-save tests that exercise the full save body.)*
...
- [X] T111 [P] Update `internal/audience/domain/subscription_page.go` (and the matching command/query handlers used by Phase 6) so the per-subscription-page "visible profile fields" picker reads from `subscriber_fields` (built-in pseudo-rows + tenant registry rows) — Phase 6 and Phase 7 share one canonical list per FR-016b *(Implemented via [SaveSubscriptionPageHandler.validateFieldKeys](../../internal/audience/app/command/save_subscription_page.go) — injects `domain.FieldRepository`, rejects any `FormField.Key` not in built-in slugs ∪ tenant registry rows.)*
- [X] T112 [P] Update `frontend/src/routes/t/$slug/public-pages/$id.tsx` (the Phase 6 subscription-page editor) to read the visible-field options from `tp(slug).subscriberFields.list` (the same source the merge-tag picker uses); deprecate any inline field-list source *(The freeform `key` text input becomes a `<select>` populated from `queryKeys.subscriberFields(slug)`; the label still allows a per-page override but auto-fills from the registry's displayName. A stale slug (registry entry deleted after the page was saved) stays visible with a "no longer in registry" marker so the operator can re-pick.)*
- [X] T113 [P] Backend integration test (cross-context): create a registry entry, verify it appears in BOTH the `/subscriber-fields` response AND the Phase 6 subscription-page's allowed-fields list; delete it, verify it disappears from both *(Lives at [subscription_page_fields_cross_context_test.go](../../internal/api/subscription_page_fields_cross_context_test.go).)*

### Observability & audit

- [X] T114 [P] Add structured logging fields (`tenant_id`, `actor_id`, `request_id`, `warnings_count`) to all new endpoints in `internal/api/handlers/` and in the BFF Nitro routes (`frontend/src/server/routes/`) per Constitution V; the BFF generates `request_id` if absent and forwards it to Go via `X-Request-Id` *(Go side: new [request_id.go](../../internal/api/request_id.go) middleware reads/generates `X-Request-Id`, echoes it on the response, and stores it in context; `requestAttrs(ctx)` builds the standard slog field set (tenant_id, actor_id, actor_kind, request_id) every Phase 7 handler appends via `s.logEvent(...)`. BFF side: `runVisualCampaignSave` / `runVisualTemplateSave` / `runRenderPreview` now always emit through a `resolveLog(...)` shim that defaults to NDJSON-on-stdout when the H3 shims don't pass a logger; `render_duration_ms` is logged alongside warnings_count.)*
- [X] T115 [P] Verify audit events `subscriber_field.{create,update,delete,reorder}`, `template.save_visual`, `campaign.save_visual` are emitted Go-side after persistence with the contract'd payload shape (no body included); the BFF does not write audit rows *(Wired `iamdomain.AuditRepository` into `service.Application` and through `api.New` so the api package owns one auditor; the new [audit.go](../../internal/api/audit.go) `s.recordAudit(...)` writes an `iam.AuditRecord` from the request context. All six handlers emit their contracted events after persistence with no body. Coverage: [visual_audit_events_test.go](../../internal/api/visual_audit_events_test.go) drives all six endpoints and asserts the rows land in `audit_log`.)*
- [X] T116 [P] Add metrics with the same labels as existing template/campaign save metrics so dashboards split visual vs raw-HTML traffic; add BFF-tier render-latency metric for `renderVisualDoc` *(No prior metrics infrastructure existed in the repo — built from scratch. Go side: new [internal/platform/metrics](../../internal/platform/metrics/metrics.go) package owns a process-local Prometheus registry, exposes `nvelope_visual_editor_saves_total{kind,result,warnings_present}` + `nvelope_subscriber_fields_mutations_total{action,result}`, mounted at `GET /metrics`. BFF side: new [frontend/src/server/metrics](../../frontend/src/server/metrics/index.ts) package with `nvelope_bff_render_duration_seconds{surface,result}` histogram + `nvelope_bff_save_attempts_total{surface,kind}` counter, mounted at `GET /metrics` via [metrics.get.ts](../../frontend/src/server/routes/metrics.get.ts).)*

### Send-pipeline end-to-end verification

- [X] T117 [P] Backend integration test in `internal/sending/...`: author a campaign visually with `{{ subscriber.first_name }}` + `{{ campaign.unsubscribe_url }}` placeholders; create two subscribers with different first names; run the send pipeline against them; assert each recipient's rendered `body_html` and `body_text` contain the correctly-substituted values; assert tracking-link rewrite and open-pixel injection from existing Phase 3 still happen on top *(Test exposed a real Phase 7 gap: the substituter (T025) was never wired into the batch worker. Wiring landed alongside the test — new `SubscriberLookup` + `TenantNameLookup` ports on the BatchWorker (in [batch_worker.go](../../internal/campaign/adapters/batch_worker.go)), composition-root bridges `NewSubscriberLookup` / `NewTenantNameLookup` in [bridges.go](../../internal/service/bridges.go), and `sendOne` now runs `sending.Substitute(...)` before the tracker rewrite so `{{ campaign.unsubscribe_url }}` becomes a real per-recipient URL the tracker leaves alone. Test at [visual_send_pipeline_test.go](../../internal/api/visual_send_pipeline_test.go).)*

### Sanitization end-to-end

- [X] T118 [P] End-to-end test (BFF → Go): POST a browser-shape `PUT /visual` with a `RawHTML` block containing a `<script>` tag; assert the BFF renders, Go's bluemonday strips it, the save response includes a `sanitizer_stripped` warning AND the persisted `body_html` contains no `<script>`. Separately POST a `render-preview` with the same content and assert the BFF's preview sanitizer emits the warning (per FR-014a — endpoint-specific warning sources). *(Go-side assertion at [visual_sanitization_e2e_test.go](../../internal/api/visual_sanitization_e2e_test.go) — stands in for the BFF by forwarding an already-rendered bodyHtml with a `<script>` tag to Go's `PUT /visual` and asserts bluemonday strips it + a `sanitizer_stripped` warning lands in the response. Required adding `json:` tags to `RenderWarning` so the API serializes camelCase keys. BFF-side assertion added to [render-preview.test.ts](../../frontend/src/server/routes/render-preview.test.ts) — drives `runRenderPreview` with a `RawHTML` block carrying a script, asserts the preview sanitizer strips it and emits the warning.)*

### Documentation & runtime config

- [X] T119 [P] Update `docs/architecture.md` if it documents the editor surface (likely yes); cross-link to [plan.md](./plan.md) and [research.md](./research.md); document the BFF render tier *(Added a BFF tier paragraph at the end of [§ 9 Frontend](../../docs/architecture.md) — names the three Nitro routes, the source layout under `frontend/src/server/`, the metrics endpoint, and the fail-closed contract.)*
- [X] T120 [P] Update `docs/implementation-plan.md` to mark Phase 7 as in-flight / delivered *(Phase 7 section in [implementation-plan.md](../../docs/implementation-plan.md) now marks the visual editor as in flight, names the BFF tier, calls out the merge-tag substituter wiring + audit-log emission, and notes the first `/metrics` endpoints on both tiers.)*

### Final validation

- [ ] T121 Run the [quickstart.md](./quickstart.md) end-to-end walkthrough manually against a local stack (`make test-db-clean && make migrate-up && go run ./cmd/api & go run ./cmd/worker & pnpm --filter ./frontend dev`); confirm each user story *(Deferred to ship-time — needs an operator at a browser; the automated suites (T122/T123) cover everything that can run headless.)*
- [X] T122 Run `make test` and `pnpm --filter ./frontend test` — both green *(Frontend: 60 files / 294 tests green. Go: every package passes in isolation; a serial `go test ./... -p 1` run is fully green. A pre-existing testcontainer contention between `internal/api/` and `test/` surfaces as a flake under full parallelism — not introduced by Phase 8.)*
- [X] T123 Run `pnpm --filter ./frontend typecheck` and `pnpm --filter ./frontend lint` — clean
