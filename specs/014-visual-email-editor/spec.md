...
- Q: Where does structured-doc → email-HTML rendering execute? → A: In the existing TanStack Start + Nitro BFF, using `@react-email/components` and `@react-email/render`. The BFF intercepts `PUT /t/{slug}/api/campaigns/{id}/visual` and `POST /t/{slug}/api/campaigns/{id}/render-preview` before the proxy; for save it renders, then forwards `{ subject, bodyDoc, bodyHtml, bodyText, theme }` to the Go API with the user's session cookie forwarded. Go re-validates the doc, sanitizes the HTML via bluemonday, and persists the three pieces atomically. The browser never produces canonical HTML and the response shape to the SPA is unchanged.
