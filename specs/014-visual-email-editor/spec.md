...
- Q: Where does structured-doc → email-HTML rendering execute? → A: In the existing TanStack Start + Nitro BFF, using `@react-email/components` and `@react-email/render`. The BFF intercepts `PUT /t/{slug}/api/campaigns/{id}/visual`, `PUT /t/{slug}/api/templates/{id}/visual`, and `POST /t/{slug}/api/render-preview` (tenant-scoped, shared by both editors — see the N4 clarification below) before the proxy; for save it renders, then forwards `{ subject, bodyDoc, bodyHtml, bodyText, theme, ifUnmodifiedSince }` to the Go API with the user's session cookie forwarded. Go re-validates the doc, sanitizes the HTML via bluemonday, and persists the three pieces atomically. The browser never produces canonical HTML and the response shape to the SPA is unchanged.
- Q: What happens when the BFF cannot reach the Go API for a side-call (subscriber-fields fetch for placeholder validation, or branding fetch for theme defaults)? → A: Fail closed. The BFF returns `502 bad_gateway` and the save aborts; the operator retries. No partial state, no silent fallback to platform defaults.
- Q: Does the BFF render path apply to visual templates (US2) as well? → A: Yes. When US2 lands, `PUT /t/{slug}/api/templates/{id}/visual` is BFF-hosted with the same orchestration shape as the campaign endpoint; the Go template save handler is amended in the same way.
- Q: Where do "sanitizer_stripped" style warnings originate under the BFF topology? → A: Endpoint-specific. Save-response warnings are emitted exclusively by Go's bluemonday pass — they reflect what was actually stripped before persistence. Preview-response warnings are emitted exclusively by the BFF — the preview iframe is rendered by the BFF and never touches Go, so the BFF runs its own sanitization over preview HTML and reports any strips. Each endpoint's warnings come from whoever sanitized.
- Q: How are audit events emitted for visual saves? → A: Audit rows stay Go-side. `campaign.save_visual` and `template.save_visual` are emitted by the Go save handler after persistence with the unchanged payload shape `{ id, warnings_count }` (warnings counted from Go's sanitizer output). The BFF does not write audit rows.
- Q: What is the BFF's structured-logging stance? → A: Mirror Go's format. The two new Nitro routes log `tenant_id`, `actor_id`, `request_id` per Constitution V. The BFF generates `request_id` if absent and propagates it to Go via `X-Request-Id` header so one user trace correlates across BFF and Go logs.
- Q: Is autosave in Phase 7 scope? → A: No — deferred to a follow-up phase (Phase 7.1). Phase 7 ships explicit save only. The previously-listed FR-008 autosave requirement and SC-008 autosave success criterion are removed from this phase's scope; the "saving / saved / saved locally" indicator set, the "session expired with recoverable draft" edge case, and the "offline, saves queued" edge case all move to the follow-up. Operators are expected to click Save; an unsaved-changes prompt on navigate-away is the only persistence-loss guard in Phase 7.
- Q: How is FR-009 multi-tab conflict enforced? → A: Optimistic concurrency on the existing `updated_at` timestamp. The save request body carries `ifUnmodifiedSince: <ISO timestamp>` populated from the value the editor saw when it loaded the row; the Go save handler compares it against the row's current `updated_at` and returns a typed `409 stale_row` error on mismatch. The BFF passes the header through transparently. The SPA surfaces a "This campaign/template was changed in another tab. Reload / Force overwrite" affordance; "Force overwrite" re-issues the save with the row's latest `updated_at`. Catches cross-tab, cross-device, and cross-browser cases uniformly. No schema migration required.
- Q: Where does sample-data placeholder substitution for the render-preview iframe execute? → A: Go-side, via a side-call from the BFF. The BFF preview route renders the structured document with `@react-email/components`, then POSTs `{ html, text, sampleSubscriber, sampleCampaign }` to a new Go endpoint `POST /api/v1/t/{slug}/substitute-sample`. Go reuses the existing send-pipeline substituter (`internal/sending/domain/substitution.go`) to resolve the sample values and returns the substituted html/text to the BFF, which then sanitizes and returns to the SPA. One substituter implementation, zero drift surface between preview and inbox. Preview is not a hot path, so the extra BFF→Go hop is acceptable. The BFF MUST NOT reimplement substitution rules in TypeScript.
- Q: Is "link" a block type in FR-002? → A: No — link is a text formatting (a mark applied to a text range), not a standalone block. FR-002's block enumeration is reworded to drop "link"; hyperlink application is covered by FR-005 (bubble-menu link control). The data-model grammar (`Mark := … | { type:"link", attrs:{ href:string } }`) is unchanged and is the source of truth.
- Q: Does FR-007 desktop/mobile preview apply to templates as well as campaigns, and if so on which endpoint? → A: Yes — preview applies to both editors. The previously-named `POST /campaigns/{id}/render-preview` is renamed to `POST /render-preview` (tenant-scoped, not row-scoped); both the campaign editor and the template editor call the same Nitro route. The endpoint's body is doc-only (`bodyDoc`, `theme`, `sample`) — it never read the campaign row anyway. One endpoint, one Nitro handler, one set of golden fixtures for both editors.
...
- The operator opens two browser tabs on the same campaign and edits both — the second save is rejected by the server with `409 stale_row` (the request's `ifUnmodifiedSince` no longer matches the row's `updated_at`), and the editor surfaces a clear "Changed in another tab/session — Reload or Force overwrite" affordance rather than silently overwriting. Same flow applies cross-device and cross-browser.
...
- The editor session is left idle for a long time and the auth session expires — explicit Save surfaces a clear "session expired, sign in again" message rather than silently losing the draft; in Phase 7 the editor warns the operator to copy/export their content before re-authenticating (cross-session recovery moves to the Phase 7.1 autosave follow-up).
...
- The operator works in the editor offline (loss of network) — typing and reordering remain responsive locally; an explicit Save while offline surfaces a clear "offline — reconnect and try Save again" error and leaves the in-memory content untouched (queued-save handling moves to the Phase 7.1 autosave follow-up).
...
- The BFF cannot reach the Go API for a side-call required by the visual save (subscriber-field registry fetch for placeholder validation, or tenant-branding fetch for theme defaults) — the save fails closed with a clear `502 bad_gateway` error; the editor's draft and structured document are preserved client-side; the operator retries. No silent fallback to platform defaults and no partially-persisted row.
...
- **FR-002**: The visual editor MUST support, at minimum, the following block types: paragraph, heading (multiple levels), bulleted list, numbered list, quote, code block, image, button, divider, and multi-column layouts of two, three, and four columns whose individual columns can themselves contain any of the other block types. Hyperlinks are applied as a text formatting (mark) via the bubble menu (per FR-005), not as a standalone block.
...
- **FR-007**: The visual editor MUST offer a desktop-width (600 px) and a mobile-width (375 px) preview that reflect how the produced HTML will render at those viewport widths. The preview surface is shared between the campaign editor and the template editor (both call the same tenant-scoped `POST /render-preview` endpoint).
- **FR-008**: *(deferred to Phase 7.1 — see [Clarifications](#clarifications))*. Phase 7 does not autosave. The visual editor MUST persist content only on explicit Save. The editor MUST surface a "saving" and "saved" state for the explicit-save round-trip and MUST warn before discarding unsaved changes when the operator attempts to navigate away.
- **FR-009**: The visual editor MUST surface a clear conflict message (rather than silent overwrite) when the same campaign or template has been saved from another browser tab, device, or session since the operator opened it. The save request MUST carry an `ifUnmodifiedSince` timestamp populated from the row's `updated_at` at load time; the server MUST reject the save with a typed `409 stale_row` error when that timestamp does not match the row's current `updated_at`. The editor MUST then surface a "Changed in another tab/session — Reload or Force overwrite" affordance; choosing "Force overwrite" re-issues the save against the row's latest `updated_at`.
...
- **FR-013b**: The render from structured document to email-ready HTML and plain-text MUST happen server-side at save time. The frontend MUST POST only the structured document on save; the server tier MUST render, sanitize, and persist all three pieces atomically within the same logical save. The render step runs in the workspace's BFF (TanStack Start + Nitro) using a server-controlled email-component library; the BFF forwards the rendered HTML + plain-text alongside the structured document to the Go API for sanitization and persistence. The browser MUST NEVER produce the canonical HTML. The send pipeline (`cmd/worker`) MUST NOT render visual documents — it consumes the stored HTML and plain-text as-is.
...
- **FR-014a**: "Sanitizer stripped" style warnings surfaced to the operator MUST originate from whoever actually removed the content. Save-response warnings MUST be emitted exclusively by the Go API's sanitizer pass over the rendered HTML before persistence. Preview-response warnings MUST be emitted exclusively by the BFF's sanitizer pass over the previewed HTML (since the preview path never reaches Go). Each endpoint's warnings reflect what was actually stripped from its output; the BFF MUST NOT emit predictive warnings on the save path and Go MUST NOT receive preview requests.
...
- **SC-008**: *(deferred to Phase 7.1 alongside FR-008)*. In Phase 7, the editor MUST prompt before discarding unsaved changes on navigate-away in 100% of cases; cross-tab-close recovery is out of scope and is delivered by the autosave follow-up.
